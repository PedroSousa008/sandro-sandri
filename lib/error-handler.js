...
function sendSecureError(res, error, statusCode = 500, userMessage = null, errorCode = null) {
    ...
    try {
        ...
        let safeMessage;
        try {
            safeMessage = userMessage || sanitizeErrorMessage(error, 'An unexpected error occurred. Please try again later.');
        } catch (sanitizeError) {
            console.error('Error sanitizing error message:', sanitizeError);
        ...
        if (process.env.NODE_ENV !== 'production' && process.env.VERCEL_ENV !== 'production') {
            ...
            try {
                ...
                // Ignore debug error, just don't include debug field
            }
        }
        
        // Explicitly set content-type to JSON before sending
        try {
            res.setHeader('Content-Type', 'application/json');
            res.status(statusCode).json(response);
        } catch (sendError) {
            // If we can't send JSON, try to send plain text as last resort
        ...
        if (!res.headersSent) {
            try {
                ...
                res.status(statusCode || 500).json({
                    ...
                    message: userMessage || 'An unexpected error occurred. Please try again later.'
                });
            } catch (finalError) {
                console.error('Failed to send fallback error response:', finalError);
            }
        }
    }
}
