...
<body style="margin: 0; padding: 0; font-family: 'Arapey', 'Georgia', serif; background-color: #f5f5f0; color: #1a1a2e;">
    ...
    try {
        ...
        // Check for errors in response (Resend returns errors in result.error)
        ...
        // Check if result has id (success) or data.id
        ...
        if (messageId) {
            ...
            console.warn('⚠️ Resend response does not contain message ID');
            console.warn('   Response:', result);
            // Still return success if no error, but log warning
            return { success: true, messageId: null, warning: 'No message ID in response' };
        }
    } catch (error) {
