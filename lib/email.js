    ...
    try {
        ...
        if (result && result.error) {
            ...
            if ((errorMsg.includes('domain') || errorMsg.includes('verified')) && 
                fromEmail !== 'onboarding@resend.dev') {
                console.log('🔄 Domain verification error in response. Retrying with onboarding@resend.dev...');
                fromEmail = 'onboarding@resend.dev';
                try {
                    result = await resend.emails.send({
                        from: fromEmail,
        ...
        if (error.message && (error.message.includes('domain') || error.message.includes('verified'))) {
            ...
            throw new Error('Unable to send verification email. Please check your Resend API key and domain configuration. For testing, ensure RESEND_FROM_EMAIL is not set or is set to onboarding@resend.dev');
        }
