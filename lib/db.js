...
// On Vercel, use /tmp for writable storage. Otherwise use .data
const isVercel = process.env.VERCEL === '1' || process.env.VERCEL_ENV;
const DB_DIR = isVercel ? '/tmp/sandro-sandri-data' : path.join(process.cwd(), '.data');
...
const DB_FILES = {
    ...
    atlas: path.join(DB_DIR, 'atlas.json'),
    userData: path.join(DB_DIR, 'user-data.json')
};
...
async function initDb() {
    ...
    try {
        ...
        try {
            ...
            console.error('Failed to initialize webhook events:', error);
        }
    }
    
    // Initialize atlas data if it doesn't exist
    try {
        await fs.access(DB_FILES.atlas);
    } catch {
        try {
            await fs.writeFile(DB_FILES.atlas, JSON.stringify({}, null, 2));
        } catch (error) {
    ...
    try {
        await fs.access(DB_FILES.userData);
    } catch {
        try {
            await fs.writeFile(DB_FILES.userData, JSON.stringify({}, null, 2));
        } catch (error) {
...
async function getInventory() {
    try {
        ...
        try {
            ...
            console.warn('Inventory file not found, returning empty inventory:', error.message);
            return {};
        }
    } catch (error) {
...
async function getAtlasData() {
    try {
        await initDb();
        try {
            const data = await fs.readFile(DB_FILES.atlas, 'utf8');
            return JSON.parse(data);
        } catch (error) {
            // File doesn't exist, return empty atlas data
            console.warn('Atlas data file not found, returning empty data:', error.message);
            return {};
        }
    } catch (error) {
        console.error('Error in getAtlasData:', error);
        return {};
    }
}
...
async function saveAtlasData(atlasData) {
    try {
        await initDb();
        await fs.writeFile(DB_FILES.atlas, JSON.stringify(atlasData, null, 2));
    } catch (error) {
...
// Get user data (cart, profile, etc.)
async function getUserData() {
    try {
        await initDb();
        try {
            const data = await fs.readFile(DB_FILES.userData, 'utf8');
            return JSON.parse(data);
        } catch (error) {
            // File doesn't exist, return empty user data
            console.warn('User data file not found, returning empty data:', error.message);
            return {};
        }
    } catch (error) {
...
async function saveUserData(userData) {
    try {
        await initDb();
        await fs.writeFile(DB_FILES.userData, JSON.stringify(userData, null, 2));
    } catch (error) {
