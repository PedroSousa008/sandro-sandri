...
// SECURITY: Owner password - can be set via environment variable for extra security
// If not set, uses hardcoded value (only in backend, never exposed to frontend)
const OWNER_PASSWORD = process.env.OWNER_PASSWORD || 'Sousa10Pedro';
...
async function verifyOwnerCredentials(email, password, securityAnswer) {
    ...
    // SECURITY: Never log the actual password or compare in logs
    ...
    if (!ownerUser.passwordHash) {
        ...
        const updatedUserData = await db.getUserData();
        ownerUser = updatedUserData[OWNER_EMAIL];
    }
    
    // Verify password against stored hash
    console.log('🔐 Verifying password against stored hash...');
    // SECURITY: Never log passwords or comparison details
    const isValid = await bcrypt.compare(password, ownerUser.passwordHash);
    if (!isValid) {
        console.error('❌ Password verification failed');
        // SECURITY: Never log actual passwords in error messages
        return { valid: false, error: 'Invalid credentials' };
    }
...
module.exports = {
    ...
    // SECURITY: OWNER_PASSWORD is NOT exported - stays internal to this module only
};
