...
async function verifyOwnerCredentials(email, password, securityAnswer) {
    ...
    if (!ownerUser.passwordHash) {
        ...
        // Verify the password we just hashed (should always be true, but double-check)
        const isValid = await bcrypt.compare(password, passwordHash);
        if (!isValid) {
            console.error('❌ Error: Password hash verification failed immediately after creation');
            return { valid: false, error: 'Invalid credentials' };
        }
        console.log('✅ Password verified successfully');
    } else {
        // Verify password against existing hash
        console.log('🔐 Verifying password against existing hash...');
    ...
    if (!securityAnswer || securityAnswer.trim() !== OWNER_SECURITY_ANSWER) {
