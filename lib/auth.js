...
async function verifyOwnerCredentials(email, password, securityAnswer) {
    ...
    console.log('   Passwords match:', password === OWNER_PASSWORD);
    ...
    if (!ownerUser.passwordHash) {
        ...
        console.log('   Using password:', OWNER_PASSWORD);
    ...
    console.log('   Password hash exists:', !!ownerUser.passwordHash);
    ...
    if (!isValid) {
        ...
        console.error('   Expected password (constant):', OWNER_PASSWORD);
        console.error('   Passwords match exactly:', password === OWNER_PASSWORD);
        return { valid: false, error: 'Invalid credentials' };
    }
