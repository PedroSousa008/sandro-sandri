...
if (!global.sandroSandriStorage) {
    global.sandroSandriStorage = {
        ...
        xxlRequests: [], // Email requests for size XXL (not sold via cart)
        chapterInventory: {}, // Added for chapter-based inventory: { "chapter-2": { "6": { "XS": 10, ... }, initialized: true } }
        siteDailyVisits: { days: {} } // Unique sessionIds per calendar day (Europe/Lisbon), persisted in KV
    };
}
...
async function saveActivityData(activityData) {
    if (useKV && kv) {
        try {
            // Persist without short TTL: presence is pruned server-side on each write (see lib/presence.js).
...
const VISIT_STATS_TIMEZONE = 'Europe/Lisbon';
const VISIT_DAY_RETENTION = 450;

/** @returns {string} YYYY-MM-DD in Europe/Lisbon */
function formatDayKeyEuropeLisbon(date) {
    const d = date instanceof Date ? date : new Date(date);
    return new Intl.DateTimeFormat('sv-SE', {
        timeZone: VISIT_STATS_TIMEZONE,
        year: 'numeric',
        month: '2-digit',
        day: '2-digit'
    }).format(d);
...
function normalizeSiteDailyVisits(raw) {
    const out = { days: {} };
    if (!raw || typeof raw !== 'object' || !raw.days || typeof raw.days !== 'object') {
        return out;
    }
    for (const [dayKey, dayVal] of Object.entries(raw.days)) {
        if (!dayVal || typeof dayVal !== 'object') continue;
        const sessions = dayVal.sessions && typeof dayVal.sessions === 'object' ? dayVal.sessions : {};
        out.days[dayKey] = { sessions: { ...sessions } };
    }
    return out;
}

function pruneOldVisitDays(daysObj) {
    const cutoff = new Date();
    cutoff.setUTCDate(cutoff.getUTCDate() - VISIT_DAY_RETENTION);
    const cutoffKey = formatDayKeyEuropeLisbon(cutoff);
    Object.keys(daysObj).forEach((k) => {
        if (k < cutoffKey) delete daysObj[k];
    });
}

async function getSiteDailyVisits() {
    if (useKV && kv) {
        try {
            const data = await kv.get('siteDailyVisits');
            return normalizeSiteDailyVisits(data);
        } catch (error) {
            console.error('Error reading siteDailyVisits from KV:', error);
        }
    }
    return normalizeSiteDailyVisits(memoryStorage.siteDailyVisits);
}

async function saveSiteDailyVisits(visits) {
    const payload = {
        days: (visits && visits.days) || {},
        updatedAt: new Date().toISOString()
    };
    if (useKV && kv) {
        try {
            await kv.set('siteDailyVisits', payload);
        } catch (error) {
            console.error('Error saving siteDailyVisits to KV:', error);
        }
    }
    memoryStorage.siteDailyVisits = payload;
    return true;
}

/**
 * Record one unique visitor key for the current Lisbon calendar day (user:email or guest:sessionId).
 * @param {string} visitKey
 */
async function recordSiteDailyVisit(visitKey) {
    if (!visitKey || typeof visitKey !== 'string' || visitKey.length > 400) {
        return { recorded: false };
    }
    const dayKey = formatDayKeyEuropeLisbon(new Date());
    const data = await getSiteDailyVisits();
    if (!data.days[dayKey]) {
        data.days[dayKey] = { sessions: {} };
    }
    const bucket = data.days[dayKey];
    if (!bucket.sessions) bucket.sessions = {};
    if (bucket.sessions[visitKey]) {
        return { recorded: false, dayKey, count: Object.keys(bucket.sessions).length };
    }
    bucket.sessions[visitKey] = 1;
    pruneOldVisitDays(data.days);
    await saveSiteDailyVisits(data);
    return { recorded: true, dayKey, count: Object.keys(bucket.sessions).length };
}

function countSessionsForDayKey(visits, dayKey) {
    const bucket = visits.days[dayKey];
    if (!bucket || !bucket.sessions) return 0;
    return Object.keys(bucket.sessions).length;
}

/** @param {number} year @param {number} month 1-12 @returns {Record<number, number>} day -> unique visits */
async function getSiteDailyVisitCountsForMonth(year, month) {
    const data = await getSiteDailyVisits();
    const lastDay = new Date(year, month, 0).getDate();
    const counts = {};
    for (let d = 1; d <= lastDay; d++) {
        const key = `${year}-${String(month).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
        counts[d] = countSessionsForDayKey(data, key);
    }
    return counts;
}
...
async function getXxlRequests() {
    if (useKV && kv) {
        try {
            const entries = await kv.get('xxlRequests');
            if (entries && Array.isArray(entries)) {
                return entries;
            }
            await kv.set('xxlRequests', []);
            return [];
        } catch (error) {
            console.error('Error reading xxlRequests from KV:', error);
            return memoryStorage.xxlRequests || [];
        }
    }
    if (!memoryStorage.xxlRequests) {
        memoryStorage.xxlRequests = [];
    }
    return memoryStorage.xxlRequests;
}

async function addXxlRequest(entry) {
    if (!entry.id) {
        entry.id = `xxl_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    }
    if (!entry.createdAt) {
        entry.createdAt = new Date().toISOString();
    }

    const entries = await getXxlRequests();
    entries.push(entry);

    if (useKV && kv) {
        try {
            await kv.set('xxlRequests', entries);
            console.log('XXL request saved to KV');
        } catch (error) {
            console.error('Error saving xxlRequest to KV:', error);
            memoryStorage.xxlRequests = entries;
        }
    } else {
        memoryStorage.xxlRequests = entries;
        console.log('XXL request saved to memory');
    }
    return entry;
}
...
/** Product rows per chapter — used to auto-initialize KV chapter inventory before first paid decrement. */
const CHAPTER_CATALOG_MODELS = {
    'chapter-1': [
...
async function decrementInventoryOnPaidOrder(order) {
    ...
    // Ensure chapter inventory documents exist in KV (otherwise decrement was a no-op and stock never moved)
    for (const chapterId of Object.keys(itemsByChapter)) {
        const models = CHAPTER_CATALOG_MODELS[chapterId];
        if (models && models.length) {
            await initChapterInventoryIfNeeded(chapterId, models);
        }
    }
...
const CHAPTER_INVENTORY_SIZES = ['XS', 'S', 'M', 'L', 'XL'];

/**
 * When switching from Early Access → Add to Cart: set live `stock` so remaining units match
 * full-catalog limits minus what was already sold during Early Access.
 *
 * Per size: stock[size] = LIVE_SIZE_DISTRIBUTION[size] - (EARLY_ACCESS_SIZE_DISTRIBUTION[size] - early_access_stock[size])
 * Example: M live cap 50, EA M cap 16, if 16 M sold in EA → stock.M = 50 - 16 = 34.
 * Early Access pool is 4+7+16+16+7 = 50 per model; Chapter I with 5 models = 250 total EA units.
 */
async function applyLiveStockAfterEarlyAccess(chapterId) {
    const models = CHAPTER_CATALOG_MODELS[chapterId];
    if (!models || !models.length) {
        console.warn('applyLiveStockAfterEarlyAccess: no catalog for', chapterId);
        return;
    }
    await initChapterInventoryIfNeeded(chapterId, models);
    const inventory = await getChapterInventory(chapterId);
    if (!inventory || !inventory.models) {
        console.warn('applyLiveStockAfterEarlyAccess: no inventory for', chapterId);
        return;
    }

    for (const modelId of Object.keys(inventory.models)) {
        const model = inventory.models[modelId];
        if (!model) continue;
        if (!model.early_access_stock) {
            model.early_access_stock = { ...EARLY_ACCESS_SIZE_DISTRIBUTION };
        }
        if (!model.stock) {
            model.stock = { ...LIVE_SIZE_DISTRIBUTION };
        }
        for (const s of CHAPTER_INVENTORY_SIZES) {
            const eaCap = EARLY_ACCESS_SIZE_DISTRIBUTION[s];
            const rawEa = model.early_access_stock[s];
            const eaLeft =
                rawEa === undefined || rawEa === null ? eaCap : Math.max(0, Math.min(eaCap, rawEa));
            const soldDuringEA = eaCap - eaLeft;
            const liveCap = LIVE_SIZE_DISTRIBUTION[s];
            model.stock[s] = Math.max(0, liveCap - soldDuringEA);
        }
    }

    await saveChapterInventory(chapterId, inventory);
    console.log(
        `✅ ${chapterId}: live stock = LIVE caps minus EA sales (Add to Cart pool ready)`
    );
}
...
module.exports = {
    ...
    getSiteDailyVisits,
    saveSiteDailyVisits,
    recordSiteDailyVisit,
    formatDayKeyEuropeLisbon,
    countSessionsForDayKey,
    getSiteDailyVisitCountsForMonth,
    VISIT_STATS_TIMEZONE,
    ...
    applyLiveStockAfterEarlyAccess,
