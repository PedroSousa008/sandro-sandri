...
if (!global.sandroSandriStorage) {
    global.sandroSandriStorage = {
        ...
        chapterInventory: {} // Added for chapter-based inventory: { 'chapter-1': { productId: { XS: 10, ... }, ... }, ... }
    };
}
...
async function getChapterInventory() {
    if (useKV && kv) {
        try {
            const inventory = await kv.get('chapterInventory');
            if (inventory) {
                return inventory;
            }
            // Initialize empty object if not found
            await kv.set('chapterInventory', {});
            return {};
        } catch (error) {
            console.error('Error reading chapterInventory from KV:', error);
            return memoryStorage.chapterInventory || {};
        }
    }
    if (!memoryStorage.chapterInventory) {
        memoryStorage.chapterInventory = {};
    }
    return memoryStorage.chapterInventory;
}

async function saveChapterInventory(inventory) {
    if (useKV && kv) {
        try {
            await kv.set('chapterInventory', inventory || {});
            console.log('Chapter inventory saved to KV');
        } catch (error) {
            console.error('Error saving chapterInventory to KV:', error);
            memoryStorage.chapterInventory = inventory || {};
        }
    } else {
        memoryStorage.chapterInventory = inventory || {};
        console.log('Chapter inventory saved to memory');
    }
    return true;
}
