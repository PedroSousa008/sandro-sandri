...
try {
    ...
    if (vercelKV && process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN) {
        kv = vercelKV;
        useKV = true;
        console.log('Using Vercel KV for persistent storage');
    }
} catch (error) {
    console.warn('Vercel KV not available, using in-memory storage (data will not persist across invocations)');
}
