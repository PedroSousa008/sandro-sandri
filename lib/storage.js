...
function getDefaultInventory() {
    ...
    // LIVE stock: 150 pieces total per model (XS: 10, S: 20, M: 50, L: 50, XL: 20)
    // Early Access: 50 pieces total per model, can be distributed in any combination
    // respecting maximum limits: XS <= 10, S <= 20, M <= 50, L <= 50, XL <= 20
    // Default Early Access distribution: XS: 3, S: 7, M: 15, L: 15, XL: 10 (total: 50)
    // Owner can adjust this distribution in Admin Page as long as total = 50 and each size <= max
    return {
        1: {
...
// Validate Early Access stock distribution
// Rules:
// 1. Total must equal 50 pieces
// 2. Each size must not exceed its maximum: XS <= 10, S <= 20, M <= 50, L <= 50, XL <= 20
function validateEarlyAccessStock(earlyAccessStock) {
    if (!earlyAccessStock) return { valid: false, error: 'Early access stock is missing' };
    
    const maxLimits = { XS: 10, S: 20, M: 50, L: 50, XL: 20 };
    ...
    for (const size of sizes) {
        const stock = earlyAccessStock[size] || 0;
        const max = maxLimits[size];
        
        if (stock < 0) {
            errors.push(`${size} cannot be negative`);
        }
        if (stock > max) {
            errors.push(`${size} exceeds maximum of ${max} (got ${stock})`);
        }
    ...
    if (total !== 50) {
        errors.push(`Total must be 50 pieces (got ${total})`);
    }
...
async function getInventory() {
    ...
    if (useKV && kv) {
        try {
            inventory = await kv.get('inventory');
            if (inventory) {
                console.log('📦 Inventory loaded from KV');
                
                // Ensure all products (including Chapter II) have stock initialized
                let needsUpdate = false;
                for (const productId in defaultInventory) {
                    if (!inventory[productId]) {
                        console.log(`📦 Product ${productId} missing from inventory, initializing...`);
                        inventory[productId] = defaultInventory[productId];
                        needsUpdate = true;
                    } else {
                        // Ensure live_stock structure exists
                        if (!inventory[productId].live_stock) {
                            console.log(`📦 Product ${productId} missing live_stock, initializing...`);
                            inventory[productId].live_stock = defaultInventory[productId].live_stock;
                            needsUpdate = true;
                        }
                        // Ensure early_access_stock structure exists
                        if (!inventory[productId].early_access_stock) {
                            console.log(`📦 Product ${productId} missing early_access_stock, initializing...`);
                            inventory[productId].early_access_stock = defaultInventory[productId].early_access_stock;
                            needsUpdate = true;
                        }
                    }
                }
                ...
                if (needsUpdate) {
                    console.log('📦 Updating inventory with missing products...');
                    await kv.set('inventory', inventory);
                    console.log('✅ Inventory updated with all products');
                }
    ...
    if (!memoryStorage.inventory) {
        memoryStorage.inventory = defaultInventory;
        ...
        // Ensure all products exist in memory storage too
        let needsUpdate = false;
        for (const productId in defaultInventory) {
            if (!memoryStorage.inventory[productId]) {
                memoryStorage.inventory[productId] = defaultInventory[productId];
                needsUpdate = true;
            }
        }
        if (needsUpdate && useKV && kv) {
            try {
                await kv.set('inventory', memoryStorage.inventory);
            } catch (error) {
...
function getProducts() {
    return [
        ...
        // Chapter II Products
        { id: 6, name: "Maldives", sku: "SS-006", sizes: ["XS", "S", "M", "L", "XL"] },
        { id: 7, name: "Palma Mallorca", sku: "SS-007", sizes: ["XS", "S", "M", "L", "XL"] },
        { id: 8, name: "Lago di Como", sku: "SS-008", sizes: ["XS", "S", "M", "L", "XL"] },
        { id: 9, name: "Gisele", sku: "SS-009", sizes: ["XS", "S", "M", "L", "XL"] },
        { id: 10, name: "Pourville", sku: "SS-010", sizes: ["XS", "S", "M", "L", "XL"] }
    ];
}
...
// Get default chapters structure (NEW SYSTEM)
function getDefaultChapters() {
    return [
        { id: 'chapter_i', name: 'Chapter I', created: true, mode: 'add_to_cart', updatedAt: new Date().toISOString() },
        { id: 'chapter_ii', name: 'Chapter II', created: false, mode: 'add_to_cart', updatedAt: new Date().toISOString() },
        { id: 'chapter_iii', name: 'Chapter III', created: false, mode: 'add_to_cart', updatedAt: new Date().toISOString() },
        { id: 'chapter_iv', name: 'Chapter IV', created: false, mode: 'add_to_cart', updatedAt: new Date().toISOString() },
        { id: 'chapter_v', name: 'Chapter V', created: false, mode: 'add_to_cart', updatedAt: new Date().toISOString() },
        { id: 'chapter_vi', name: 'Chapter VI', created: false, mode: 'add_to_cart', updatedAt: new Date().toISOString() },
        { id: 'chapter_vii', name: 'Chapter VII', created: false, mode: 'add_to_cart', updatedAt: new Date().toISOString() },
        { id: 'chapter_viii', name: 'Chapter VIII', created: false, mode: 'add_to_cart', updatedAt: new Date().toISOString() },
        { id: 'chapter_ix', name: 'Chapter IX', created: false, mode: 'add_to_cart', updatedAt: new Date().toISOString() },
        { id: 'chapter_x', name: 'Chapter X', created: false, mode: 'add_to_cart', updatedAt: new Date().toISOString() }
    ];
}

// Convert chapter ID to number (chapter_i = 1, chapter_ii = 2, etc.)
function chapterIdToNumber(chapterId) {
    const romanMap = {
        'i': 1, 'ii': 2, 'iii': 3, 'iv': 4, 'v': 5,
        'vi': 6, 'vii': 7, 'viii': 8, 'ix': 9, 'x': 10
    };
    const roman = chapterId.replace('chapter_', '');
    return romanMap[roman] || 0;
}

// Get active chapter ID (the most recent chapter with created=true)
function getActiveChapterId(chapters) {
    if (!chapters || !Array.isArray(chapters)) return null;
    
    // Find all created chapters
    const createdChapters = chapters.filter(ch => ch.created === true);
    if (createdChapters.length === 0) return null;
    
    // Sort by chapter number (most recent = highest number)
    createdChapters.sort((a, b) => {
        const aNum = chapterIdToNumber(a.id);
        const bNum = chapterIdToNumber(b.id);
        return bNum - aNum; // Most recent first (highest number)
    });
    
    return createdChapters[0].id;
}

// Get default chapter modes structure (LEGACY - keeping for backward compatibility)
function getDefaultChapterModes() {
    return {
        chapter_i: {
            created: true,
            mode: 'LIVE', // 'WAITLIST', 'EARLY_ACCESS', or 'LIVE'
...
async function getSiteSettings() {
    if (useKV && kv) {
        try {
            ...
            if (settings) {
                ...
                if (!settings.chapter_modes) {
                    settings.chapter_modes = getDefaultChapterModes();
            ...
            const defaultSettings = {
                ...
                chapter_modes: getDefaultChapterModes(),
                updatedAt: new Date().toISOString()
            };
    ...
    } else if (!memoryStorage.siteSettings.chapter_modes) {
        memoryStorage.siteSettings.chapter_modes = getDefaultChapterModes();
    }
    return memoryStorage.siteSettings;
}
