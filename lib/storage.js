...
async function getSiteSettings() {
    if (useKV && kv) {
        try {
            ...
            // Initialize with default LIVE mode
            ...
            return memoryStorage.siteSettings || { commerce_mode: 'LIVE', updatedAt: new Date().toISOString() };
        }
    }
...
async function saveSiteSettings(settings) {
    if (useKV && kv) {
        try {
            await kv.set('siteSettings', settings || { commerce_mode: 'LIVE', updatedAt: new Date().toISOString() });
            console.log('Site settings saved to KV');
        } catch (error) {
            console.error('Error saving siteSettings to KV:', error);
            memoryStorage.siteSettings = settings || { commerce_mode: 'LIVE', updatedAt: new Date().toISOString() };
        }
    } else {
        memoryStorage.siteSettings = settings || { commerce_mode: 'LIVE', updatedAt: new Date().toISOString() };
