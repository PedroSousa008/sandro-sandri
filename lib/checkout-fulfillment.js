...
async function fulfillCheckoutSession(session) {
    ...
    let customerEmail = (meta.customerEmail && String(meta.customerEmail).trim()) || '';
    if (!customerEmail && session.customer_email) {
        customerEmail = String(session.customer_email).trim();
    }
    ...
    if (!customerEmail) {
        customerEmail = `order-${String(session.id).replace(/[^a-z0-9_]/gi, '')}@checkout.local`;
    }

    let customerName = (meta.customerName && String(meta.customerName).trim()) || '';
    if (!customerName && session.customer_details && session.customer_details.name) {
